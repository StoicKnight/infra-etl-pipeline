...
from src.etl.load import load_hosts_netbox
    ...
    async with NetBoxAPIClient(
        ...
        async with await SaltAPIClient.create(
            ...
            async with XenAPIClient(
                base_url=str(settings.xen["cyprus"].api.base_url),
                token=settings.xen["cyprus"].api.token,
            ) as xen_client:
                try:
                    ...
                    log.info("Load Hosts to NetBox...")
                    await load_hosts_netbox(netbox_client, xen_client)
